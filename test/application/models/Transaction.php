...
class Transaction extends CI_Model {
    ...
    public function getAll($orderBy, $orderFormat, $start, $limit) {
        if ($this->db->platform() == "sqlite3") {
            ...
            $this->db->select('transactions.ref, transactions.totalMoneySpent, transactions.modeOfPayment, transactions.staffId,
                transactions.transDate, transactions.lastUpdated, transactions.amountTendered,transactions.deliveryAmount, transactions.changeDue,
     ...
     * @return boolean
     */
    public function add($_iN, $_iC, $desc, $q, $_up, $_tp, $_tas, $_at, $_cd, $_mop, $_tt, $ref, $_va, $_vp, $damt, $dp,$da, $cn, $cp, $ce) {
        $data = [ 
                'itemName' => $_iN, 
                'itemCode' => $_iC,
                'description' => $desc, 
    ...
    public function transSearch($value) {
        ...
        if ($run_q->num_rows() > 0) {
            //echo "<pre>"; print_r($run_q->result()); die;
            return $run_q->result();
        }
        else {
            return FALSE;
